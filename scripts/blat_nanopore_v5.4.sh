...
# This is version 5.4 of the Nanopore circRNA dataction pipeline
...
rm base_list_exon-match.temp base_list_exon-match.temp2 base_list_exon-match.temp3.bed base_list_exon-match.temp4.bed base_list_exon-match.annot.prefilter.bed

### for the reads that do not match exons I check for 95% similarity to circBase, circAtlas or CIRCpedia circRNA. If this is found, the annotated circRNA entry defines boundaries.
...
# Annotating with 95% similar circRNAs
bedtools map -f 0.95 -F 0.95 -c 4 -o last -a $sample.scan.circRNA.psl.annot.combine.sort.temp -b $circBase | awk '!($0 in a) {a[$0];print}' > $sample.scan.circRNA.psl.annot.combine.sort.temp2
bedtools map -f 0.95 -F 0.95 -c 4 -o last -a $sample.scan.circRNA.psl.annot.combine.sort.temp2 -b $circAtlas | awk '!($0 in a) {a[$0];print}' > $sample.scan.circRNA.psl.annot.combine.sort.temp3
bedtools map -f 0.95 -F 0.95 -c 4 -o last -a $sample.scan.circRNA.psl.annot.combine.sort.temp3 -b $CIRCpedia | awk '!($0 in a) {a[$0];print}' > $sample.scan.circRNA.psl.annot.combine.circID.bed
...
rm $sample.scan.circRNA.psl.annot.combine.txt $sample.scan.Potential_multi-round_circRNA.psl.merge.bed $sample.scan.Potential_multi-round_circRNA.sort.bam.bai
...
rm base_list_exon-match.bed base_list_exon-match.temp2.bed no_exon_match_reads.bed circBase.no_exon_match_reads_circIDs.txt circAtlas.no_exon_match_reads_circIDs.txt
rm base_list_no-exon.cirBaseID.annot.prefilter.bed CIRCpedia.no_exon_match_reads_circIDs.txt
